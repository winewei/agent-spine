...
每交付一个 change（spec）**成功 archived** 后，从「工作 agent 表现」视角派生一份单-change 收尾回执：`spec-report.md`（人读、简要）+ `spec-report.json`（审计契约源、实用优先）+ 一条 `spec.report` telemetry 事件（喂 `/spine-analyze`）。三产物从**同一派生对象**产出。纯确定性，不新增采集、不 spawn agent。本 change 只覆盖 **archived delivery receipt**；非 archived 终态不生成报告。
