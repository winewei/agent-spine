...
`npc lessons record --seq N` SHALL 在目标 change（由 `--seq N` 指定其在 `plan_order` 中的序号，`N` 与 fix round 数字无关）已到达 `archived` 终态后可用，读取该 change 的 `<base>/events.jsonl`，筛选全部 `kind == "phase.exit" && phase` 匹配正则 `^fix-r\d+$`（即该 change 全部 fix 轮，round 数字不做任何过滤）`&& status == "done"` 的记录，按 phase 中的 round 数字升序提取其 `categories_scanned` / `regressions_added` / `notes` 字段，拼接为一条 markdown 段落追加到 `<run_dir>/lessons.md`（不存在则创建）。相同输入 MUST 产出相同追加结果（不调用任何 LLM，不做语义摘要）。
...
`npc lessons record` 的实现 MUST NOT 读取、打开或引用 `round-N.review.json`、`round-N.focus.md`、`round-N.spec-review.json` 或其中的 category / finding / rubric 原文；提炼字段 MUST 仅取自 `fix-rN` phase.exit 事件里 fixer 通过 RESULT 行自报的 `categories_scanned` / `regressions_added` / `notes`。
