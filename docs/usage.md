...
`npc` 单独可用，但它作为**自主 harness 底座**的价值要靠 **CLI + playbooks + 项目上下文片段 三层一起配** 才能兑现。本文给出可直接照做的完整步骤。宿主可以是任意 agent CLI（Claude Code / Kimi CLI / Qwen Code / Codex / OpenCode / …）；宿主间差异只在层 2 的物化目标与层 3 的文件名。

---

## 层 1：装 `npc` 命令（机器级，所有宿主 session 共享）

`npc` 是从 agent-spine 仓库安装出来的 CLI 命令，代码在 `src/npc`。直接从 GitHub 远程安装，无需 clone：

```bash
uv tool install --force --from git+https://github.com/winewei/agent-spine.git npc
npc --version          # 应输出 npc 1.10.0
```
