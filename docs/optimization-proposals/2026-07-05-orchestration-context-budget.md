...
四条不变量真实落到了代码层（`verify routing` / `verify tests` / `verify manifest` / 结构化契约），不是口号。
...
| 偏差记账（归因落 telemetry） | **该有，且是前置** | 是原则要求的"先收证据后建轨"的证据收集步骤 |
...
| **sub-agent** | token 重的分析：DAG 抽取、架构师裁定、implement、**失败诊断（triage）** | 读大量原文、结论可压成一行 JSON |
...
- **`npc state note --text "..."`**（追加式编排日志）：承载两类内容——编排器自己的意图备忘（为什么这么排、悬而未决什么）与**人的中途转向指令（steering）**。`status --brief` 带出最近 N 条未消费 note；主循环在 change 边界消费。一条基建同时解决 §1.2 判定表的 steering 与"意图跨 compaction"。
...
- **明确不做**：自动归因裁定、升级阶梯 L2/L3 自动化——等这份记账跑出 hotspots 再按原则建轨。
...
**开发纪律（一条判据）**：*skill 文档里任何超过三行的 bash 块，都是一个待下沉的 bug。*
...
| 自适应 stale 阈值 / 预算控制器 | 定位实际转向无人值守（不变量 3 原文条件） |
