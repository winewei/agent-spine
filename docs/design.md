...
首期范围：把 `/new-plan-changes` skill（1280 行）下沉，产出 `npc` 子工具与 `/new-plan-changes-v2` skill（目标 ≤ 500 行）。
...
P5/P6 端到端实测后暴露了两类结构性冗余，分别在 0.2 与 0.3 解决。

### 11.1 0.2：子命令自包含（`run.json` + `active.json`）

**问题**：v0.1 调用模式要求 `eval "$(npc init --shell-exports --auto)"`，把 13 个 `NPC_*` 环境变量注入到当前 shell。Claude Code 的 Bash 工具每次调用是独立 shell，env 不跨调用持久化，主 session 每个 Bash call 都被迫重复 eval init，浪费 token 与轮次。
...
### 11.2 0.3：高层 pipeline（codex / openspec 下沉）

**问题**：即便 0.2 自包含化之后，LLM 仍在做大量"数据搬运"——`jq` 拆 init JSON 取 `REPO_ROOT` / `SCHEMA_PATH`、字面注入 `codex exec --cd ... --output-schema ...`、串调 `npc review parse → update-trend → check-stale → phase exit` 四步、以及 archive 段的 `precheck → openspec validate → openspec archive → git commit → phase exit → state set-progress` 六步。
...
src/agent_spine/npc/
├── (0.1 模块全部保留)
├── pipeline.py      # 0.3 新增：review run / archive run / implement record / fix record
└── paths.py         # 0.2 重写：run.json / active.json + load_paths 优先级 resolve
```

测试套件从 0.1 的 104 个 → 0.2 的 115 个 → 0.3 的 127 个。

### 11.4 1.0：Sub-agent prompt 下沉（major bump）

**问题**：v2 skill 把 §A Implementer / §B Fixer 两段 prompt 模板（各 ~2500 tokens）写在 skill 文档里，主 session 用 Write 拼模板 → 写到 `$BASE/...prompt.md` → 整文件内容塞进 `Agent(prompt=...)` 字段。同一份模板在主 session context 里出现两次（Write input + Agent input），单次 sub-agent 调用注意力开销 ~5000 tokens。整 run（6 changes × 平均 4 轮）累计 ~120k tokens 浪费。
...
- 模板字段（双产物契约、RESULT schema、修复规则 A-D）作为"项目级硬契约"内置，不暴露为可配置项；契约升级需升 npc 版本
...
- 0.x 子命令全部保留，调用方可平滑升级
- skill 文档历史上的 §A/§B 段落删除，由人类可审计的契约速记表 + 指向 `templates.py` 的链接替代
- 版本号跳 0.3 → 1.0：standalone CLI 的对外契约（命令 + stdout schema）首次稳定承诺，**major bump 标记可生产使用**

**对主 session 的影响**（实测）：

| 单次 sub-agent 调用 | 0.3 写法 | 1.0 写法 | 缩减 |
|---|---|---|---|
...
src/agent_spine/npc/
├── (0.1 / 0.2 / 0.3 模块全部保留)
├── templates.py     # 1.0 新增：§A Implementer / §B Fixer / spawn-prompt 模板纯函数
└── agent.py         # 1.0 新增：agent prompt render / spawn-prompt CLI handler
```

测试套件从 0.3 的 127 个 → 1.0 的 **159 个**（+32：13 templates + 19 agent）。
