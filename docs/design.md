...
### 11.12 失败可归因、打地鼠治理与波次流水化（1.7.1）

来源：一次多 change v4 长时 run 的复盘。四类问题叠加：(a) 两处 npc 缺陷在 run 中热修——strict 模式 verify tests 把目标仓库既有的测试失败判为回归并 revert；`openspec archive` 静默中止后 `git commit` 报 "nothing to commit"（走 stdout）被记为 `git-commit-failed` 且 stderr 为空；(b) 多个 change 连续多轮 review，每轮 high finding 都是**新的**——同一条跨文件不变量被 reviewer 每轮报一处、fixer 每轮只扫 finding 所在文件，双方增量推进形成"打地鼠"；(c) 波次过薄（多个单 change 波），下一波 implement 要等上一波最慢 change 的多轮修复结束；(d) 安装的 npc 来自未合并分支，仓库 main 落后。1.7.1 对应四项：

- **失败事件必带诊断**（不变量 2 的可归因要求）：`archive` 的 git commit 失败合并捕获 stdout+stderr，识别 `nothing-to-commit`；`integrate.verify_tests_failed` 事件带 `cmd / tests.new_failures / tail`，state 装订 `verify_tests_detail`。`[verify].test_baseline = "diff"` 基线对照模式收编（061ac96）；`openspec archive` 静默中止以 change 目录是否仍在为准（aafe6cc）。
...
## 1.9.0：统一的后台任务观测与终态信号

`npc monitor` 成为主 session 唯一的后台观测通道，覆盖 sub-agent、远程作业与后台命令，替代会话内的临时等待脚本与外部看门狗。

**职责边界**：主 session 是登记表的唯一写入者和唯一决策者，负责登记、确认、关闭与再调度；sub-agent 只在 RESULT 中回报作业句柄，不写登记表、不在自身会话中等待作业。monitor 只采集证据、判定规则并生成待处理动作，不关闭任务、不停止进程、不调度 agent、不修改 Git 或业务 state。

**任务类型**：`agent` 可被询问，沿用 CHECK_IN / CONTROL_REQUIRED；`job` 无询问对象，无进展时产生 STALL。job 登记时必须提供完成判定、存活判定或截止时间之一，保证每个任务都存在终态检测路径。

**终态信号与关闭分离**：完成判定成立（`DONE_SIGNAL`）、存活判定连续失败且未完成（`EXITED_SIGNAL`）、超过截止时间（`DEADLINE`）只是关闭依据。任务进入 `signaled` 后停止探测，直到主 session 核验后 `finish` / `cancel`，或以 `intervene` 恢复探测、以有界 `wait` 延后重新提示。完成标记是必要条件而非充分条件，结果正确性由主 session 核验，因此 monitor 不自动关闭任务，也不设自动过期。

**送达语义**：每个任务同一时刻只有一个待处理动作，未确认的动作出现在后续每一次快照中，构成至少一次送达；宿主后台监视工具到期重新挂接不丢失事项，因此不引入带序号的事件日志与订阅游标。follow 仅在出现新动作或动作转为无进展时输出，未处理动作按固定间隔重复提醒；主 session 自身的登记、确认、关闭不触发输出，空闲时不输出心跳。

**询问节奏**：上一询问周期内证据有变化的 agent，询问间隔放宽到 30 分钟；证据停滞的 agent 保持 10 分钟。证据变化只说明有工作产出，不证明收敛，因此询问只放宽、不取消。

**执行与存储**：检查命令在锁外执行，结果在第二次加锁时应用，慢速远程探测不阻塞确认与登记。状态保持 JSON 快照加 flock 与原子替换；决策记录追加写入 `monitor.history.jsonl`。单个 run 的任务规模为数百条，完整读写开销为毫秒级，不需要数据库。
