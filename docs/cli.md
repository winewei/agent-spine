...
`--section S`（1.10.0）只输出按需分节。常驻正文只保留每次都要执行的主流程；低频分支（spine-run 的 `plan` / `monitor` / `publish` / `recovery` / `finish`）放在包资源 `playbooks/<name>/<S>.md`，在触发条件出现时才取用。分节不随 `install` 写入宿主，内容始终与已安装的 npc 版本一致。
...
`--format line`（`follow` 默认，1.10.0）把同一结果压成一行纯文本，只保留决策所需字段，完整结构留在 `monitor.json`，由 `list --open` 按需读取：
