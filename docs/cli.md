...
- `--config PATH`（可选）：显式指定 TOML 配置文件；省略则按 `<repo>/.npc/config.toml` → `~/.config/npc/config.toml` 标准查找链
...
### `npc implement run --seq N [--change-id ID] [--backend claude|mimo|codex] [--timeout N] [--config PATH]`（1.3+）
...
- `--backend {claude,mimo,codex}`（可选）：覆盖 coder 后端。优先级：`--backend` > `[coder.phase].implement`（per-phase 覆盖）> `[coder].backend`（全局）> 默认 `claude`。**MiMo 默认不启用**，仅显式指定 `mimo` 时才路由过去
- `--timeout N`（可选）：coder 子进程超时秒数
- `--config PATH`（可选）：显式 TOML 配置路径

**做什么**：

1. 加载配置，解析 `selected backend`
2. `phase enter implement`
3. 渲染 §A Implementer 模板到 `$BASE/implement.prompt.md`
4. 按 backend 起子进程：`claude`/`mimo` 走 headless `claude -p <prompt> [--model M] --permission-mode bypassPermissions`（`mimo` 额外注入 `~/.config/npc/mimo.env` 解析出的环境变量覆盖到子进程 env）；`codex` **尚未实现**
...
**exit**：`0` 成功；`1` 业务失败（`implement record` 校验不过等）；`2` 配置错误（`ConfigError`，或 `--backend codex` 尚未实现）；`3` 环境错（缺 state / MiMo env 文件读取失败，如 chmod 权限问题）；`4` 依赖缺失（coder 可执行文件未在 PATH 中找到，或 `mimo.env` 文件本身不存在）
...
### `npc fix run --seq N --round M [--change-id ID] [--backend claude|mimo|codex] [--timeout N] [--config PATH]`（1.3+）
...
**exit**：`0` 成功；`1` 业务失败；`2` 配置错误 / 缺 `--round` / `--backend codex` 尚未实现；`3` 环境错；`4` 依赖缺失
...
环境前置体检：git / openspec / codex / claude / jq / portable-timeout（PATH 或 `~/.local/bin` 自举）/ review schema 自举情况 / mimo.env 成本路由 / npc config 可加载性 / `docs/principles.md`。除 `git` 外全部是 warn 级、不阻塞。

**做什么**：对 `_BIN_CHECKS`（`git` 必备，`openspec`/`codex`/`claude`/`jq` 可选）逐个查 PATH；`portable-timeout` 额外查 `~/.local/bin` 自举位置并校验可执行位；`schema` 检查 `~/task_log/.new-plan-review-schema.json` 是否存在且为合法 JSON；`mimo.env` 检查 `~/.config/npc/mimo.env` 是否存在可读；`config` 尝试 `load_config`（失败降级 warn，不阻塞）；`principles.md` 检查 `<repo>/docs/principles.md`。
...
1. `backend_unsupported` / `engine_unsupported`：`coder.backend`（含每个 `[coder.phase]` per-phase 覆盖）与 `review.engine` 必须在各自支持列表内
2. `gen_not_orthogonal`：coder 在用的任一后端与 review 解析到同一执行身份（都是 claude 且 `bin`+`model` 相同；或都是 mimo）→ 等于自己评自己
3. `mimo_exec_only`：review 路由（`engine` / `claude_bin` / `claude_model`）含 `mimo` 字样 → 无条件顶层挡
