...
| 版本 | 关键变化 |
|---|---|
| **1.9.1** | 主 session context 预算：`monitor tick` / `follow` 新增 `--format line` 一行纯文本输出（`monitor: N open, M pending | #ID KIND TASK [-> HANDLE] [asked] [idle Nm] [probe-error]; ... | detail: npc monitor list --open`，`*` 标记新动作），默认仍为 `--format json`，stdout 契约不变；spine-run 显式使用 `--format line`；`playbook show --section` 按需分节，spine-run 常驻正文由 32KB 降到约 20KB，恢复/发布回执/monitor 事件/多 change 计划/收尾移入分节；`playbook list` 增 `sections` 字段 |
