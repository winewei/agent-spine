...
5. 校验 commit 在本 run 工作树 HEAD 祖先链上（`git merge-base --is-ancestor <commit> HEAD`）；失败 → reason=commit-not-on-run-branch（linked worktree 与主 checkout 共享对象库，coder cwd 漂移到其他 checkout 提交时 cat-file 探测不出来——此门确定性拒绝；fix record 同）
...
| `scripts/check_spec.py --change <id>`（仓库本地资产，非 `npc` 子命令） | 静态语义 lint（五条 warning-only 规则），由 `[spec_review] gate_cmd` 调用 | 作为 gate_cmd 被本节的 `npc spec review run` 调用，但 npc 本身不持有其规则语义 |
| `npc spec interrogate\|write\|fix\|review`（本节） | 一句话目标 → 模式盘问 → openspec change artifact → 强制独立语义评审 + 固定轮次 fix 循环 | 是 |

### `npc spec interrogate run --change <id> [--goal "<原始一句话目标>"] [--config PATH]`

**模式盘问**：write 轮的硬前置。渲染 interrogate 轮 prompt（`<base>/pattern-interrogation.prompt.md`），要求 `spine-spec-writer` 在动笔撰写任何 artifact 之前，先枚举「仓库里已有的最近似实现」，产出 `openspec/changes/<id>/pattern-interrogation.md`（三个 H2 段落：`## Analogs` 文件+函数级引用、`## Assumptions`、`## Open Questions`）。渲染前先调 `check_routing(cfg)`，与 `spec write|fix run` **复用同一套** `spec_` 前缀路由规则集（不引入第二套后端白名单）；违规立即以 `spec_routing_violation` 拒绝、不渲染任何 prompt。恒返回 `.deferred == true`。`--goal` 透传语义与 `spec write run` 一致。prompt 正文 **MUST NOT** 含任何 `SPEC_REVIEW_SCHEMA.category` 枚举或 `round-*.spec-review.json` 原文（不变量 1）。超时预算复用既有四件套，phase 名 `spec_interrogate`。

### `npc spec interrogate record --change <id> --result "<RESULT行>"`

装订 interrogate 轮 RESULT。必需键：`change`/`artifacts`/`summary`（**无 `validate`**——`pattern-interrogation.md` 不是 openspec validate 认识的 artifact 类型）。越界/意外 commit 拦截同 `spec write record`。装订后**独立解析** `pattern-interrogation.md` 的 `## Open Questions` 段落顶层 `- ` bullet 数作为 `.open_questions`（**不采信** writer 自报，不变量 2）。产物缺失 → `pattern_interrogation_missing`；缺三个必需 H2 标题任一 → `pattern_interrogation_missing_section`（输出 `missing_sections` 列缺失标题名，不静默按 0 处理）；`## Open Questions` 标题存在但无 bullet → `.ok == true`、`.open_questions == 0`。该结构判据与 `spec write run` 硬门**共用同一实现**，对同一 fixture 给出相同缺失标题集合，不出现「record 通过但 write gate 拒绝」的分歧。

### `npc spec interrogate decide --change <id> --decisions-md "<裁决原文>"`

纯机械文本追加（交互档用户裁决后调用）：把 `--decisions-md` 原文追加进 `pattern-interrogation.md` 末尾的 `## User Decisions (Interactive)` 段。npc **不解析、不改写、不做语义判断** 其内容。一次性、不覆盖：文件已含该段 → `decisions_already_recorded`、逐字节不改动；产物不存在 → `pattern_interrogation_missing`。
...
**模式盘问硬前置门（排在路由校验之后）**：routing 合法后，检查 `openspec/changes/<id>/pattern-interrogation.md` 是否存在——不存在 → `pattern_interrogation_missing`，**不渲染任何 write prompt**；存在则进一步检查三个必需 H2 标题（`## Analogs`/`## Assumptions`/`## Open Questions`）是否齐全，缺任一 → `pattern_interrogation_missing_section`（同样不渲染）。此门对分支 A（`--goal` 透传）与分支 B（补全既有 change）一视同仁。路由违规恒优先于本门（错误标识恒为 `spec_routing_violation`）。write prompt 另新增 `pattern-interrogation.md` 为必读输入，并含消费指令（按是否含 `## User Decisions (Interactive)` 标题这一机械判据把 Open Questions/Assumptions/裁决段原样带入 design.md 的 `## Pattern Mapping`）与多落点确定性搜索命令枚举指令。
...
调用方判定是否达到 `max_rounds` 时，**必须**直接读本命令 stdout 的 `.max_rounds` 字段（success 分支已把已加载的 `[spec_review].max_rounds` 原样透传）——`npc verify routing` 只 emit 路由不变量字段（`ok`/`coder_backend`/`review_engine`/`violations`），从不含 `max_rounds`，不能作为该配置的真相源。

**`ok == false` 的非门失败场景**（`gate_failed` 为空，`.blocking` 键不存在）：`dependency_missing`（`portable_timeout`/`codex`/`claude` 二进制缺失）、`<engine>-exec-failed`（引擎超时/非零退出/产物缺失）、`invalid_spec_review_schema`（引擎输出不符 `SPEC_REVIEW_SCHEMA`）。调用方**必须先判定 `.ok`**，再决定是否读 `.blocking`——`.ok == false` 时 `jq '.blocking // 0'` 之类的缺省值写法会把「评审根本没跑完」误判为「评审通过（blocking=0）」，不得放行进入 clean 分支（round 4 finding F1 的根因）。
...
[worktree]
provision_cmd = "pnpm install --frozen-lockfile --prefer-offline"
# worktree 创建成功后在其中执行的一次性预备命令（补齐 node_modules 等 gitignore 依赖，
# 让 coder 与 record 的 rerun-tests 能在 worktree 内跑测试）。默认空 = 不执行。
# shlex 拆分后直接 exec（不经 shell），600s 超时；失败仅 [npc:warn] 不阻塞 init，
# 结果记入 init JSON 的 provision 字段（{ran, ok, rc, cmd, tail}；未配置 {ran:false}）。
```

### 超时预算：`npc agent timeout-budget|record-timeout --change <id> --phase spec_interrogate|spec_write|spec_fix-rN`
...
- **`npc doctor`**：环境前置体检（git/openspec/codex/claude/jq/portable-timeout/schema/mimo.env/config/principles.md/openspec/project.md）。输出 `{ok, checks[], summary}`；required(git) 缺失 exit 3。`openspec/project.md` 为 warn 级共读上下文体检（存在/非空/含约定段落，`required: false`，不影响退出码）。
