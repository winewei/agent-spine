...
# new-plan-changes-v4 → spine-run（1.9.0）
...
执行前检查 `npc --version` 至少为 **1.9.0**，且 `npc monitor register --help` 支持 `--kind` / `--done`、`npc change run --help` 支持新协议。若 CLI 低于 1.9.0，明确指出版本不匹配；不能用旧 CLI 默默执行新 skill。更新 CLI 后重新安装配套 playbook。
