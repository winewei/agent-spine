...
# new-plan-changes-v4 → spine-run（1.8.1）
...
执行前检查 `npc --version` 至少为 **1.8.1**，且 `npc monitor --help` 与 `npc change run --help` 支持新协议。若 CLI 仍是 1.7.1/1.8.0，明确指出版本不匹配；不能用旧 CLI 默默执行新 skill。更新 CLI 后重新安装配套 playbook。
...
- `npc init` 后、首个分析/执行 agent 前启动唯一 monitor。登记全部角色，每 10 分钟核查产出，15 分钟无推进时诊断；询问、回复、干预均记录回执。用可唤醒主 session 的通知机制或至多 60 秒有界等待，不把后台日志误认为自动控制。
