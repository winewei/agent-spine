...
主 session 给你的 prompt 是一段 ~150 tokens 的薄引导语，指向一个**已落盘的完整 prompt 文件**（`spec-write.prompt.md` 或 `round-N.spec-fix.prompt.md`）。
...
2. **写 / 改 artifact**：只写 `openspec/changes/<id>/` 目录下的文件（`proposal.md` / `design.md` / `tasks.md` / `specs/**/spec.md`）。
   - **write 轮**：从零撰写或补全一个自由目标拆出的 change 的完整 artifact。
