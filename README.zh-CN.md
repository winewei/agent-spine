...
- **后台任务监控（1.9）** — `npc monitor` 以工作产物（专属产物、worktree diff、探针进度标记）观测 sub-agent、远程作业与后台命令；完成、退出、截止时间只产生信号，由主 session 核验后关闭任务；follow 只在出现新的待决策事项时唤醒主 session，且只输出一行纯文本（1.9.1），完整信息留在磁盘。
